// ============================================
// CATEGORY PAGE - VIOLA STORE (With Dynamic Navbar & Category Images)
// ============================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('orderForm')?.addEventListener('submit', (e) => { e.preventDefault(); const fullName = document.getElementById('fullName')?.value.trim() || ''; const phone = document.getElementById('phone')?.value.trim() || ''; const city = document.getElementById('city')?.value || ''; const address = document.getElementById('address')?.value.trim() || ''; if (!fullName || !phone || !city || !address) { showToast('⚠️ يرجى ملء جميع الحقول المطلوبة', true); return; } if (!/^05\d{8}$/.test(phone)) { showToast('⚠️ رقم الهاتف يجب أن يبدأ بـ 05 ويتكون من 10 أرقام', true); return; } showSuccessModal(); document.getElementById('orderForm')?.reset(); });
});
